...
///  const float min_strip_cut;
  ...
  MonitorElement* meMuonISO_time_muon_reco_;
  MonitorElement* meMuonISO_time_trk_reco_;
  MonitorElement* meMuonISO_time_muon_sim_;
  MonitorElement* meMuonISO_time_trk_sim_;

  MonitorElement* meMuonISO_have_time_muon_reco_;
  MonitorElement* meMuonISO_have_time_trk_reco_;
  MonitorElement* meMuonISO_have_time_muon_sim_;
  MonitorElement* meMuonISO_have_time_trk_sim_;

  MonitorElement* meMuonISO_dt_muon_trk_reco_1_;
  MonitorElement* meMuonISO_dt_muon_trk_reco_2_;
  MonitorElement* meMuonISO_dt_muon_trk_sim_;
  MonitorElement* meMuonISO_dtSig_muon_trk_reco_;
  MonitorElement* meMuonISO_dtSig_muon_trk_sim_;

  MonitorElement* meMuonISO_mva_muon_reco_;
  MonitorElement* meMuonISO_mva_trk_reco_;
  MonitorElement* meMuonISO_mva_muon_sim_;
  MonitorElement* meMuonISO_mva_trk_sim_;

  MonitorElement* meMuonISO_dz_mus_;
  MonitorElement* meMuonISO_dz_mus_EB_;
  MonitorElement* meMuonISO_dz_mus_EE_;
  MonitorElement* meMuonISO_dxy_mus_;
  MonitorElement* meMuonISO_dxy_mus_EB_;
  MonitorElement* meMuonISO_dxy_mus_EE_;
...
//              [](reco::Muon mu){return mu.passed(reco::Muon::CutBasedIdLoose) && mu.isGlobalMuon();});
  ...
  */
  ...
  //for (int iVtx = 40; iVtx < (int)vertices.size(); iVtx++) {
    ...
    if (!vertex.isFake() && vertex.ndof() >= 4) {
      Vtx_chosen = vertex;
      cout << iVtx << "th Vertex is chosen" << endl;
      break;
    }
  }
  ...
  int nmus_Sig_EB=0, nmus_Sig_EE=0;
  int nmus_Bkg_EB=0, nmus_Bkg_EE=0;
  ...
  for (const auto& muon : localMuonCollection) {
    ...
    /*
    // test
    if (muon.pt() < 10 || std::abs(muon.eta()) > 2.4) continue;
//    meMuonISO_dz_mus_->Fill(muon_track_source_dz);
//    if (muon.
...
/*
    int muon_SigTrkIdx = -1;
    int muon_TrkIdx = -1;
    double eta_diff = 99999.;
    double eta_Sigdiff = 99999.;
    for(const auto& trackGen : *GenRecTrackHandle) {
      muon_TrkIdx++;
      const reco::TrackRef muon_SigTrkRef_check(GenRecTrackHandle, muon_TrkIdx);
      double dr = reco::deltaR(trackGen.momentum(), muon.combinedMuon()->momentum());
      if(dr < 0.1) {
        double deta = std::abs(muon.combinedMuon()->eta() - muon_SigTrkRef_check->eta());
        eta_diff = deta;
        if(eta_diff < eta_Sigdiff) {
          eta_Sigdiff = eta_diff;
          muon_SigTrkIdx = muon_TrkIdx;
        }
      }
    }
*/
    ...
    //const reco::TrackRef muon_SigTrkRef2 = muon.track();  //yes
    //const reco::TrackRef muon_SigTrkRef2 = muon.combinedMuon();  //no
    //const reco::TrackRef muon_SigTrkRef2 = muon.globalTrack();  //no
    ...
    meMuonISO_time_muon_reco_->Fill(t0Pid[muon_SigTrkRef]);
    meMuonISO_mva_muon_reco_->Fill(mtdQualMVA[muon_SigTrkRef]);
    if(t0Pid[muon_SigTrkRef]==0) meMuonISO_have_time_muon_reco_->Fill(0);
    else meMuonISO_have_time_muon_reco_->Fill(1);
    ...
    meMuonISO_time_muon_sim_->Fill(tsim_muon);
    if(tsim_muon==-1) meMuonISO_have_time_muon_sim_->Fill(0);
    else meMuonISO_have_time_muon_sim_->Fill(1);
//    cout << "muon.motherPdgId             : " << muon.motherPdgId << endl;
    ...
    if (muon_SigTrkRef.isNonnull()) {
      ...
      if (muon_Prompt) {
        ...
        if (Barrel_muon) {
          ...
          meMuon_pt_tot_Sig_EB_->Fill(muon.pt());
          meMuon_pt_sim_tot_Sig_EB_->Fill(muon_sim_pt);
          meMuon_eta_tot_Sig_EB_->Fill(std::abs(muon.eta()));
          meMuon_phi_tot_Sig_EB_->Fill(muon.phi());
          nmus_Sig_EB++;
        } else {
          // All selected muon information for efficiency plots later
          meMuon_pt_tot_Sig_EE_->Fill(muon.pt());
          meMuon_pt_sim_tot_Sig_EE_->Fill(muon_sim_pt);
          meMuon_eta_tot_Sig_EE_->Fill(std::abs(muon.eta()));
          meMuon_phi_tot_Sig_EE_->Fill(muon.phi());
          nmus_Sig_EE++;
        }
      } else {
        // For background (non-promt)
        if (Barrel_muon) {
          meMuon_pt_tot_Bkg_EB_->Fill(muon.pt());
          meMuon_pt_sim_tot_Bkg_EB_->Fill(muon_sim_pt);
          meMuon_eta_tot_Bkg_EB_->Fill(std::abs(muon.eta()));
          meMuon_phi_tot_Bkg_EB_->Fill(muon.phi());
          nmus_Bkg_EB++;
        } else {
          meMuon_pt_tot_Bkg_EE_->Fill(muon.pt());
          meMuon_pt_sim_tot_Bkg_EE_->Fill(muon_sim_pt);
          meMuon_eta_tot_Bkg_EE_->Fill(std::abs(muon.eta()));
          meMuon_phi_tot_Bkg_EE_->Fill(muon.phi());
          nmus_Bkg_EE++;
        }
      }
	...
	meMuonISO_time_trk_reco_->Fill(t0Pid[trackref_general]);
	meMuonISO_mva_trk_reco_->Fill(mtdQualMVA[trackref_general]);
	if(t0Pid[trackref_general]==0) meMuonISO_have_time_trk_reco_->Fill(0);
	else meMuonISO_have_time_trk_reco_->Fill(1);
	...
	meMuonISO_time_trk_sim_->Fill(tsim_trk);
	if(tsim_trk==-1) meMuonISO_have_time_trk_sim_->Fill(0);
	else meMuonISO_have_time_trk_sim_->Fill(1);
	...
	if((t0Pid[muon_SigTrkRef]!=0) && (t0Pid[trackref_general]!=0)) meMuonISO_dt_muon_trk_reco_1_->Fill(std::abs(t0Pid[trackref_general] - t0Pid[muon_SigTrkRef]));
	    ...
	    meMuonISO_dt_muon_trk_sim_->Fill(dt_sim_sigTrk);
	    meMuonISO_dtSig_muon_trk_sim_->Fill(dt_sim_sigTrk_signif);
	    ...
	    meMuonISO_dt_muon_trk_reco_2_->Fill(dt_sigTrk);
	    meMuonISO_dtSig_muon_trk_reco_->Fill(dt_sigTrk_signif);
          ...
          if (optionalPlots_) {
            for (long unsigned int i = 0; i < (pT_bins_dt_distrb.size() - 1); i++) {
              //stuff general pT
              if (muon.pt() > pT_bins_dt_distrb[i] && muon.pt() < pT_bins_dt_distrb[i + 1]) {
            ...
            for (long unsigned int i = 0; i < (eta_bins_dt_distrib.size() - 1); i++) {
              //stuff general eta
              if (std::abs(muon.eta()) > eta_bins_dt_distrib[i] && std::abs(muon.eta()) < eta_bins_dt_distrib[i + 1]) {
...
//	  cout << "Vtx" << endl;
      ...
      if (optionalPlots_) {
        for (long unsigned int i = 0; i < N_tracks_MTD.size(); i++) {
          ...
          rel_pT_sum_sim_MTD[i] = pT_sum_sim_MTD[i] / muon_sim_pt;
        }
        // now compute the isolation
        rel_pT_sum_noMTD = pT_sum_noMTD / muon.track()->pt();

        rel_pT_sum_gen = pT_sum_gen / muon_sim_pt;
      }

      for (long unsigned int i = 0; i < N_tracks_MTD_significance.size(); i++) {
        rel_pT_sum_MTD_significance[i] = pT_sum_MTD_significance[i] / muon.track()->pt();
        rel_pT_sum_sim_MTD_significance[i] = pT_sum_sim_MTD_significance[i] / muon_sim_pt;
      }
      ...
      if (muon_Prompt) {  // promt part
        ...
        if (Barrel_muon) {
          ...
          if (rel_pT_sum_noMTD < rel_iso_cut_) {  // filling hists for iso efficiency calculations
            meMuon_pt_noMTD_Sig_EB_->Fill(muon.pt());
            meMuon_eta_noMTD_Sig_EB_->Fill(std::abs(muon.eta()));
            meMuon_phi_noMTD_Sig_EB_->Fill(muon.phi());
          }
          if (optionalPlots_) {
            for (long unsigned int k = 0; k < Ntracks_EB_list_Sig.size(); k++) {
              if (rel_pT_sum_MTD[k] < rel_iso_cut_) {
                Muon_pT_MTD_EB_list_Sig[k]->Fill(muon.pt());
                Muon_eta_MTD_EB_list_Sig[k]->Fill(std::abs(muon.eta()));
                Muon_phi_MTD_EB_list_Sig[k]->Fill(muon.phi());

                Muon_pT_sim_MTD_EB_list_Sig[k]->Fill(muon_sim_pt);
              }
            }
            if (rel_pT_sum_gen < rel_iso_cut_) {
              meMuon_pt_gen_Sig_EB_->Fill(muon_sim_pt);
          ...
          for (long unsigned int k = 0; k < Ntracks_EB_list_Significance_Sig.size(); k++) {
            if (rel_pT_sum_MTD_significance[k] < rel_iso_cut_) {
              Muon_pT_MTD_EB_list_Significance_Sig[k]->Fill(muon.pt());
              Muon_eta_MTD_EB_list_Significance_Sig[k]->Fill(std::abs(muon.eta()));
              Muon_phi_MTD_EB_list_Significance_Sig[k]->Fill(muon.phi());
            }
            if (optionalPlots_ and rel_pT_sum_sim_MTD_significance[k] < rel_iso_cut_)
              Muon_pT_sim_MTD_EB_list_Significance_Sig[k]->Fill(muon_sim_pt);
          }
          ...
          if (rel_pT_sum_noMTD < rel_iso_cut_) {  // filling hists for iso efficiency calculations
            meMuon_pt_noMTD_Sig_EE_->Fill(muon.pt());
            meMuon_eta_noMTD_Sig_EE_->Fill(std::abs(muon.eta()));
            meMuon_phi_noMTD_Sig_EE_->Fill(muon.phi());
          }
          if (optionalPlots_) {
            for (long unsigned int k = 0; k < Ntracks_EE_list_Sig.size(); k++) {
              if (rel_pT_sum_MTD[k] < rel_iso_cut_) {
                Muon_pT_MTD_EE_list_Sig[k]->Fill(muon.pt());
                Muon_eta_MTD_EE_list_Sig[k]->Fill(std::abs(muon.eta()));
                Muon_phi_MTD_EE_list_Sig[k]->Fill(muon.phi());

                Muon_pT_sim_MTD_EE_list_Sig[k]->Fill(muon_sim_pt);
              }
            }
            if (rel_pT_sum_gen < rel_iso_cut_) {
              meMuon_pt_gen_Sig_EE_->Fill(muon_sim_pt);
          ...
          for (long unsigned int k = 0; k < Ntracks_EE_list_Significance_Sig.size(); k++) {
            if (rel_pT_sum_MTD_significance[k] < rel_iso_cut_) {
              Muon_pT_MTD_EE_list_Significance_Sig[k]->Fill(muon.pt());
              Muon_eta_MTD_EE_list_Significance_Sig[k]->Fill(std::abs(muon.eta()));
              Muon_phi_MTD_EE_list_Significance_Sig[k]->Fill(muon.phi());

              if (optionalPlots_ and rel_pT_sum_sim_MTD_significance[k] < rel_iso_cut_)
                Muon_pT_sim_MTD_EE_list_Significance_Sig[k]->Fill(muon_sim_pt);
            }
          }
        }
      } else {  // non-promt part
        ...
        if (Barrel_muon) {
          ...
          if (rel_pT_sum_noMTD < rel_iso_cut_) {  // filling hists for iso efficiency calculations
            meMuon_pt_noMTD_Bkg_EB_->Fill(muon.pt());
            meMuon_eta_noMTD_Bkg_EB_->Fill(std::abs(muon.eta()));
            meMuon_phi_noMTD_Bkg_EB_->Fill(muon.phi());
          }
          if (optionalPlots_) {
            for (long unsigned int k = 0; k < Ntracks_EB_list_Bkg.size(); k++) {
              if (rel_pT_sum_MTD[k] < rel_iso_cut_) {
                Muon_pT_MTD_EB_list_Bkg[k]->Fill(muon.pt());
                Muon_eta_MTD_EB_list_Bkg[k]->Fill(std::abs(muon.eta()));
                Muon_phi_MTD_EB_list_Bkg[k]->Fill(muon.phi());

                Muon_pT_sim_MTD_EB_list_Bkg[k]->Fill(muon_sim_pt);
              }
            }
            if (rel_pT_sum_gen < rel_iso_cut_) {
              meMuon_pt_gen_Bkg_EB_->Fill(muon_sim_pt);
          ...
          for (long unsigned int k = 0; k < Ntracks_EB_list_Significance_Bkg.size(); k++) {
            if (rel_pT_sum_MTD_significance[k] < rel_iso_cut_) {
              Muon_pT_MTD_EB_list_Significance_Bkg[k]->Fill(muon.pt());
              Muon_eta_MTD_EB_list_Significance_Bkg[k]->Fill(std::abs(muon.eta()));
              Muon_phi_MTD_EB_list_Significance_Bkg[k]->Fill(muon.phi());

              if (optionalPlots_ and rel_pT_sum_sim_MTD_significance[k] < rel_iso_cut_)
                Muon_pT_sim_MTD_EB_list_Significance_Bkg[k]->Fill(muon_sim_pt);
            }
          }
          ...
          if (rel_pT_sum_noMTD < rel_iso_cut_) {  // filling hists for iso efficiency calculations
            meMuon_pt_noMTD_Bkg_EE_->Fill(muon.pt());
            meMuon_eta_noMTD_Bkg_EE_->Fill(std::abs(muon.eta()));
            meMuon_phi_noMTD_Bkg_EE_->Fill(muon.phi());
          }
          if (optionalPlots_) {
            for (long unsigned int k = 0; k < Ntracks_EE_list_Bkg.size(); k++) {
              if (rel_pT_sum_MTD[k] < rel_iso_cut_) {
                Muon_pT_MTD_EE_list_Bkg[k]->Fill(muon.pt());
                Muon_eta_MTD_EE_list_Bkg[k]->Fill(std::abs(muon.eta()));
                Muon_phi_MTD_EE_list_Bkg[k]->Fill(muon.phi());

                Muon_pT_sim_MTD_EE_list_Bkg[k]->Fill(muon_sim_pt);
              }
            }
            if (rel_pT_sum_gen < rel_iso_cut_) {
              meMuon_pt_gen_Bkg_EE_->Fill(muon_sim_pt);
          ...
          for (long unsigned int k = 0; k < Ntracks_EE_list_Significance_Bkg.size(); k++) {
            if (rel_pT_sum_MTD_significance[k] < rel_iso_cut_) {
              Muon_pT_MTD_EE_list_Significance_Bkg[k]->Fill(muon.pt());
              Muon_eta_MTD_EE_list_Significance_Bkg[k]->Fill(std::abs(muon.eta()));
              Muon_phi_MTD_EE_list_Significance_Bkg[k]->Fill(muon.phi());

              if (optionalPlots_ and rel_pT_sum_sim_MTD_significance[k] < rel_iso_cut_)
                Muon_pT_sim_MTD_EE_list_Significance_Bkg[k]->Fill(muon_sim_pt);
            }
          }
        }
      }
...
void MtdMuonIsoValidation::bookHistograms(DQMStore::IBooker& ibook, edm::Run const& run, edm::EventSetup const& iSetup) {
  ...
  // test
  meMuonISO_time_muon_reco_ = ibook.book1D("Muon_Iso_time_muon_reco",
                                                "Time distribution for reco muons ;t^{reco}_{muon} (ns);Counts",
                                                2000,
                                                -10,
                                                10);
  meMuonISO_time_trk_reco_ = ibook.book1D("Muon_Iso_time_track_reco",
                                                "Time distribution for reco tracks ;t^{reco}_{track} (ns);Counts",
                                                2000,
                                                -10,
                                                10);
  meMuonISO_time_muon_sim_ = ibook.book1D("Muon_Iso_time_muon_sim",
                                                "Time distribution for sim muons ;t^{sim}_{muon} (ns);Counts",
                                                2000,
                                                -10,
                                                10);
  meMuonISO_time_trk_sim_ = ibook.book1D("Muon_Iso_time_track_sim",
                                                "Time distribution for sim tracks ;t^{sim}_{track} (ns);Counts",
                                                2000,
                                                -10,
                                                10);
  meMuonISO_have_time_muon_reco_ = ibook.book1D("Muon_Iso_whether_muon_have_reco_time",
                                                "Check on whether reco muon has timing information",
                                                2,
                                                0,
                                                2);
  meMuonISO_have_time_trk_reco_ = ibook.book1D("Muon_Iso_whether_track_have_reco_time",
                                                "Check on whether reco track has timing information",
                                                2,
                                                0,
                                                2);
  meMuonISO_have_time_muon_sim_ = ibook.book1D("Muon_Iso_whether_muon_have_sim_time",
                                                "Check on whether sim muon has timing information",
                                                2,
                                                0,
                                                2);
  meMuonISO_have_time_trk_sim_ = ibook.book1D("Muon_Iso_whether_track_have_sim_time",
                                                "Check on whether sim track has timing information",
                                                2,
                                                0,
                                                2);
  meMuonISO_dt_muon_trk_reco_1_ = ibook.book1D("Muon_Iso_dt_muon_track_reco_1",
                                                "dt distribution for reco track and reco muon ;|t^{reco}_{track}-t^{reco}_{muon}| (ns);Counts",
                                                2000,
                                                -10,
                                                10);
  meMuonISO_dt_muon_trk_reco_2_ = ibook.book1D("Muon_Iso_dt_muon_track_reco_2",
                                                "dt distribution for reco track and reco muon ;|t^{reco}_{track}-t^{reco}_{muon}| (ns);Counts",
                                                2000,
                                                -10,
                                                10);
  meMuonISO_dt_muon_trk_sim_ = ibook.book1D("Muon_Iso_dt_muon_track_sim",
                                                "dt distribution for sim track and sim muon ;|t^{sim}_{track}-t^{sim}_{muon}| (ns);Counts",
                                                2000,
                                                -10,
                                                10);
  meMuonISO_dtSig_muon_trk_reco_ = ibook.book1D("Muon_Iso_dtSig_muon_track_reco",
                                                "dtSig distribution for reco track and reco muon ;#sigma;Counts",
                                                2000,
                                                -10,
                                                10);
  meMuonISO_dtSig_muon_trk_sim_ = ibook.book1D("Muon_Iso_dtSig_muon_track_sim",
                                                "dtSig distribution for sim track and sim muon ;#sigma;Counts",
                                                2000,
                                                -10,
                                                10);
  meMuonISO_mva_muon_reco_ = ibook.book1D("Muon_Iso_mva_muon_reco",
                                                "MVA score of reco muons ;MVA Score;Counts",
                                                200,
                                                -1,
                                                1);
  meMuonISO_mva_trk_reco_ = ibook.book1D("Muon_Iso_mva_track_reco",
