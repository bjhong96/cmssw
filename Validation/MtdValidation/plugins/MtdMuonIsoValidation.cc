...
//              [](reco::Muon mu){return mu.passed(reco::Muon::CutBasedIdLoose) && mu.isGlobalMuon();});
  ...
  for (const auto& muon : localMuonCollection) {
    ...
    // test
//    if (muon.pt() < 10 || std::abs(muon.eta()) > 2.4) {
//      meMuonISO_dz_muon->Fill(muon_track_source_dz);
//      meMuonISO_dxy_muon->Fill(muon_track_source_dxy);
//    }
    // test end
